...
for (i, imagePath) in enumerate(imagePaths):
	...
	if len(detections) > 0:
		...
		i_detection = np.argmax(detections[0, 0, :, 2])
		confidence = detections[0, 0, i_detection, 2]
		...
		if confidence > confidence_threshold:
			...
			# the face
			box = detections[0, 0, i_detection, 3:7] * np.array([w, h, w, h])
